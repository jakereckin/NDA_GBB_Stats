...
if password == st.secrets['page_password']['PAGE_PASSWORD']:
    ...
    season_col, game_col = st.columns(spec=2)
    with season_col:
        season = st.radio(
    ...
    half_col, min_col, sec_col = st.columns(spec=3)
    with half_col:
        half = st.radio(
    ...
    second_half_col, second_min_col, second_sec_col = st.columns(spec=3)
    ...
    points_in_col, opp_points_in_col = st.columns(spec=2)
    points_out_col, opp_points_out_col = st.columns(spec=2)
    with points_in_col:
        points_in = st.number_input(
            label='Team Points When Subbed In',
            min_value=0,
            value=0,
            step=1
        )
    with points_out_col:
        points_out = st.number_input(
            label='Team Points When Subbed Out',
            min_value=0,
            value=0,
            step=1
        )
    ...
    base_calc = 18 * 60
    if half == 2:
        time_in = base_calc + minutes * 60 + seconds
    else:
        time_in = 2 * base_calc+ minutes * 60 + seconds

    if half_out == 2:
        time_out = base_calc+ minutes_out * 60 + seconds_out
    else:
        time_out = 2 * base_calc + minutes_out * 60 + seconds_out
